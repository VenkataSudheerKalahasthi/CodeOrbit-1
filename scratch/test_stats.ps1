# Test getTopicStats on all 19 topics
$d = Get-Content 'js/data.js' -Raw
$b = Get-Content 'js/basic-dsa-data.js' -Raw

$testScript = @"
function getActiveDataset(mode, PROBLEMS, BASIC_DSA_PROBLEMS) {
...
function getMatchingProblemsForTopic(topicName, dataset, allProblemsFallback) {
    ...
    let matches = filterByTopic(dataset);
    if (matches.length === 0 && allProblemsFallback && Array.isArray(allProblemsFallback)) {
        matches = filterByTopic(allProblemsFallback);
    }
    return matches;
}
...
Set-Content -Path 'scratch/test_stats.ps1' -Value $testScript
