...
const MigrationService = {
    ...
    async restoreFromCloud(userId) {
        ...
        this._restorePromise = (async () => {
            ...
            if (window.POTDManager && typeof window.POTDManager.updateCardCompletionState === 'function') {
                window.POTDManager.updateCardCompletionState();
            }
            if (window.ContestUI && typeof window.ContestUI.renderAll === 'function') {
                window.ContestUI.renderAll(false);
            }
            if (window.DailyMissionManager && typeof window.DailyMissionManager.render === 'function') {
                window.DailyMissionManager.render();
            }
            if (window.StreakCalendar && typeof window.StreakCalendar.render === 'function') {
