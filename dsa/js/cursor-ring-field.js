/* ============================================================
   CURSOR RING FIELD — OriginKit WebGL Background Engine
   Ported directly from OriginKit CursorRingField reference.
   Dark Theme ONLY — Fixed Fullscreen WebGL Background Layer.
   ============================================================ */
...
(function () {
    ...
    function initCursorRingField() {
        ...
        // Ensure canvas only created once
        let canvas = document.getElementById('originkit-cursor-ring-canvas');
        if (canvas) return;

        canvas = document.createElement('canvas');
        canvas.id = 'originkit-cursor-ring-canvas';
        ...
        try {
            ...
            console.warn("CursorRingField compile:", e.message);
            return;
        }
        ...
        // Palette setup
        const swatches = DEFAULT_COLORS.slice(0, MAX_COLORS).map(hexToRgb);
        ...
        const targetDensity = isMobile ? 130 : isTablet ? 220 : 300;

        rebuildField(targetDensity);
        ...
        const onMove = (e) => {
            pointer.x = (e.clientX / Math.max(cssW, 1)) * 2 - 1;
            pointer.y = -((e.clientY / Math.max(cssH, 1)) * 2 - 1);
            pointer.over =
        ...
        const frame = (now) => {
            ...
            // Dark theme gate: Stop rendering if light theme is active
            ...
            simTime = (simTime + dt * live.speed) % 3600;
            wander += dt * live.speed;
            ...
            let ty = wanderY;
            if (follow > 0.0001) {
                const worldX = (pointer.x * aspect * live.camDist) / projF;
                const worldY = (pointer.y * live.camDist) / projF;
            ...
            if (useSim) {
                ...
                gl.uniform1f(simProg.u.uRingWidth, live.ringWidth);
                gl.uniform1f(simProg.u.uRingWidth2, live.ringEdge);
                gl.uniform1f(simProg.u.uPush, live.push);
                gl.uniform1f(simProg.u.uTurb, live.turb);
            ...
            if (useSim) {
                ...
                gl.uniform1f(renderProg.u.uRingWidth, live.ringWidth);
                gl.uniform1f(renderProg.u.uRingWidth2, live.ringEdge);
                gl.uniform1f(renderProg.u.uTurb, live.turb);
            }

            gl.uniform1f(renderProg.u.uProjF, projF);
            gl.uniform1f(renderProg.u.uAspect, aspect);
            gl.uniform1f(renderProg.u.uCamDist, live.camDist);
            gl.uniform1f(
                renderProg.u.uPointScale,
                (cssW / 2000) * live.dotSize * dpr * 0.5
            );
            gl.uniform3fv(renderProg.u["uColors[0]"], live.colors);
            gl.uniform1i(renderProg.u.uColorCount, live.colorCount);
