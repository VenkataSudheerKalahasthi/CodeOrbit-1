...
const AdminService = {
    ...
    async checkIsAdmin() {
        ...
        try {
            ...
            if (error) {
                console.warn('[CodeOrbit Admin] user_roles query warning:', error.message);
                return false;
            }

            const isAdmin = data?.role === 'admin';
            if (!isAdmin) {
                console.info(`[CodeOrbit Admin] User (${user.email || user.id}) is authenticated with role: '${data?.role || 'none'}'. Admin authorization required.`);
            }
            return isAdmin;
        } catch (e) {
